...
const settle = (
  state: 'ok' | 'failed',
  output: string[] = [],
  changes?: Array<{sign: '+' | '-'; text: string}>
) => (items: Item[]) => {
  const at = items.map(i => i.kind === 'did' && i.state === 'running').lastIndexOf(true);
  if (at < 0) return items;
  const copy = [...items];
  copy[at] = {...(copy[at] as Extract<Item, {kind: 'did'}>), state, output, changes};
  return copy;
};

/** Code a write put in a file, as `write_file`'s own arguments state it. */
const wrote = (...lines: string[]) => lines.map(text => ({sign: '+' as const, text}));
/** And an edit, which states both sides. */
const edited = (before: string[], after: string[]) => [
...
export const SESSION: Step[] = [
  ...
  {wait: 600, change: settle('ok', [], wrote(
    'import sys',
  ...
  {wait: 200, change: doing('edit_file', 'src/extract_links.py')},
  {wait: 500, change: settle('ok', [], edited(
    ['    return urldefrag(urljoin(base, href))[0]'],
    ['    resolved = urljoin(base if base.endswith("/") else base + "/", href)',
