...
function starAt(row: number, column: number, tick: number, level: number): SkyMark | null {
  ...
  if (twinkle > 0.85) return {ch: '+', color: colour.cyanSoft};
  if (twinkle > 0.2) return {ch: ':', color: colour.dim};
...
function flowAt(line: SkyRow, column: number, tick: number, level: number): SkyMark | null {
  ...
  if (offset === 0) return {ch: '+', color: colour.cyanSoft};
  if (offset === 1 || offset === 2) return {ch: '-', color: colour.dim};
  return null;
}
